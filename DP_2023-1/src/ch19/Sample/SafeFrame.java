...
public class SafeFrame extends Frame implements Context { // 연습문제 1) ActionListener 제거 후 람다식 사용
  ...
  public SafeFrame(String title) {
    ...
    setVisible(true);
    // 리스너 설정
    // functional interface: 메소드가 딱 하나만 선언되어 있는 인터페이스(함수 역할을 하는 인터페이스)
    // functional interface 객체가 들어갈 자리에는 람다식 넣을 수 있다!
    // buttonUse.addActionListener(this); // ActionListener 객체의 actionPerformed()가
    // 호출
    // 연습문제 1 ) if문 없애기 위해 "람다식" 사용
    buttonUse.addActionListener((e) -> {
      state.doUse(this);
    }); // actionPerformed 가 하는 일
    ...
    buttonPhone.addActionListener((e) -> {
      state.doPhone(this);
    });
